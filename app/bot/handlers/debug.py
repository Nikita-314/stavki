...
def _format_live_session_start_post_cycle_report(cres: AutoSignalCycleResult) -> str:
    ...
    after_fresh = int(
        diag.get("football_live_freshness_live_events_accepted")
        or diag.get("football_live_cycle_after_send_filter")
        or 0
    )
...
async def cmd_signal_start(message: Message, sessionmaker: async_sessionmaker[AsyncSession]) -> None:
    ...
    await message.answer(
        "▶️ Старт принят. Выполняю первый live-cycle, данные появятся после fetch...",
        reply_markup=get_signal_control_keyboard(),
    )
    from app.services.football_live_session_service import FootballLiveSessionService
    ...
    try:
        ...
        await message.answer(
            "❌ Первый live-cycle завершился с ошибкой (см. серверный лог: "
            "[FOOTBALL][START_BUTTON]). Сводка со нулями не отправлялась.",
            reply_markup=get_signal_control_keyboard(),
        )
...
async def cmd_football_live_ranker_debug(message: Message, sessionmaker: async_sessionmaker[AsyncSession]) -> None:
    ...
    lines = [
        ...
        f"risk_high_count: {int(blocked_breakdown.get('risk_high_count') or 0)}",
        f"moved_to_watchlist_count: {int(blocked_breakdown.get('moved_to_watchlist_count') or 0)}",
        f"watchlist_need2_count: {int(blocked_breakdown.get('watchlist_need2_count') or 0)}",
...
async def cmd_football_live_probability_debug(message: Message, sessionmaker: async_sessionmaker[AsyncSession]) -> None:
    ...
    lines = [
        ...
        f"value_edge>=0.05: {int(diag.get('football_live_probability_value_edge_7_count') or 0)}",
        f"confidence>=55: {int(diag.get('football_live_probability_confidence_60_count') or 0)}",
