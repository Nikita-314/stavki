...
from app.schemas.period_report import (
    ...
    RealisticPeriodReport,
    RealisticPeriodReportGroupItem,
    RealisticPeriodReportOverview,
)
...
class PeriodReportService:
    ...
    async def get_realistic_period_report(self, session: AsyncSession) -> RealisticPeriodReport:
        ...
        if snapshot is not None:
            start_balance_rub = Decimal(snapshot.base_amount)
            period_started_at = snapshot.created_at
            period_label = snapshot.label
            cutoff = snapshot.created_at
            effective_dt = func.coalesce(Settlement.settled_at, Settlement.created_at)
            ...
            start_balance_rub = settings.virtual_start_balance_rub
            period_started_at = None
            period_label = "default_start_balance_rub"
            where_clause = sa.true()

        overview = await self._build_realistic_overview(
            session,
            where_clause,
            start_balance_rub=start_balance_rub,
            period_started_at=period_started_at,
            period_label=period_label,
            flat_stake=flat_stake,
        )

        by_sport = await self._group_realistic_by(session, where_clause, Signal.sport, flat_stake)
        by_bookmaker = await self._group_realistic_by(session, where_clause, Signal.bookmaker, flat_stake)
        by_market_type = await self._group_realistic_by(session, where_clause, Signal.market_type, flat_stake)

        return RealisticPeriodReport(
            overview=overview,
            by_sport=by_sport,
            by_bookmaker=by_bookmaker,
            by_market_type=by_market_type,
    ...
    async def _build_realistic_overview(
        self,
        session: AsyncSession,
        where_clause,
        *,
        ...
        return RealisticPeriodReportOverview(
            period_started_at=period_started_at,
            period_label=period_label,
            start_balance_rub=start_balance_rub,
            flat_stake_rub=flat_stake,
            total_profit_loss_rub=total_pl_rub,
            current_balance_rub=start_balance_rub + total_pl_rub,
            settled_signals_count=cnt,
            wins=wins,
            losses=losses,
            voids=voids,
    ...
    async def _group_realistic_by(
        self,
        session: AsyncSession,
        where_clause,
        group_col,
        flat_stake: Decimal,
    ) -> list[RealisticPeriodReportGroupItem]:
        pnl_rub_expr = case(
            (Settlement.result == BetResult.WIN, (Signal.odds_at_signal - 1) * flat_stake),
            (Settlement.result == BetResult.LOSE, -flat_stake),
            (Settlement.result == BetResult.VOID, 0),
        ...
        total_pl_rub = func.coalesce(func.sum(pnl_rub_expr), 0).label("total_pl_rub")
        cnt = func.count(Settlement.id).label("cnt")
        avg_pl_rub = (
            func.coalesce(func.sum(pnl_rub_expr), 0) / func.nullif(func.count(Settlement.id), 0)
        ).label("avg_pl_rub")
        ...
        stmt = (
            ...
            .where(where_clause)
            .group_by(group_col)
            .order_by(total_pl_rub.desc())
        )

        result = await session.execute(stmt)
        items: list[RealisticPeriodReportGroupItem] = []
        for r in result.all():
            key_obj = r.key
            key_str = getattr(key_obj, "value", None) or (str(key_obj) if key_obj is not None else "None")
            items.append(
                RealisticPeriodReportGroupItem(
                    ...
                    losses=int(r.losses),
                    voids=int(r.voids),
                    total_profit_loss_rub=Decimal(r.total_pl_rub),
                    avg_profit_loss_rub=Decimal(r.avg_pl_rub) if r.avg_pl_rub is not None else Decimal("0"),
                )
            )
        return items
